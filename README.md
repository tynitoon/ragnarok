...
| **v15 — general game mastery (NORTH STAR)** | New direction (owner): drop the *same* agent on **any game**, from pixels, it learns to **WIN**. Same `ConvPPONet+PPO`, generic trainer, GPU-batched games. **Pong** win-rate 0.97 (random 0.00); **Breakout** return +25.6, clears the wall ~31×/eval (random −23.5); **Snake** return +81, ~48 food/episode (random −55); **Tetris** ~63 lines/window (random 2.7) via placement-as-macro-action. **Four** distinct games mastered from pixels — incl. Tetris. Open challenge: sample-efficiency (it needs ~100× more games than a human → next: knowledge accumulation, v16). | ✅ (P0–P4) |
| **v14 — self-directed mastery from pixels (CAPSTONE)** | Given pixels and no goals, **5/5 seeds** autonomously sequence and master the **full 9-skill tree** (reaching iron-pickaxe), learning every skill **from pixels**, ~10 min. Caveat (per review): the frontier/novelty signal that picks the next skill reads inventory (a reachability *oracle*), so the curriculum *ordering* is environment-gated, not discovered from pixels — the from-pixels result is the perceptual skill-mastery + sequencing. | ✅ |
...
\* *A 2026-05-30 phase-gate review flagged that v13b's flat baseline failed under a sparse goal-only reward, so its failure might be a sparse-reward artifact rather than a depth/reuse effect. **Acted on:** a fair **shaped-reward** flat baseline (v13c — dense per-achievement reward, no reuse, generous 2.46M-step budget) was run and **still reaches only depth 0** (collect_wood) from pixels, failing every deeper target, while reuse reaches depth 6. So reuse — not reward shaping — is what unlocks the deep skills. (True pixel-based novelty, so discovery itself is from pixels rather than inventory, remains future work.) The project records and acts on such dissent rather than hiding it.*
