...
The world model converges on a CUDA GPU but diverges on the TPU after
~25-30 rollouts. The --wm-* knobs sweep the training hyperparameters to
find what stabilises it; the per-rollout `grad` column is the WM gradient
norm — a spike before the loss blows up means gradient explosion, a
smooth norm means slow numerical drift.
...
def main():
    ...
    parser.add_argument(
        ...
        help="TPU MXU fp32-matmul precision (XLA only; no-op on CUDA/CPU). "
             "'highest' is a six-pass ~fp32-faithful matmul — ruled out as "
             "the cause of the WM-on-TPU divergence, so 'default' (fast) is "
             "used here.")
    ...
    parser.add_argument("--wm-epochs", type=int, default=5,
                        help="world-model passes over each rollout.")
    ...
    print(f"  wm: lr={args.wm_lr}  grad_clip={args.wm_grad_clip}  "
          f"epochs={args.wm_epochs}")
    torch.manual_seed(0)

    rssm = RSSM(obs_dim=4, action_dim=2).to(DEVICE)
    wm = WorldModelTrainer(rssm, ReplayBuffer(), lr=args.wm_lr,
                           grad_clip=args.wm_grad_clip)
    ...
    print(f"{'rollout':>8} | {'recon':>9} | {'reward':>9} | "
          f"{'continue':>9} | {'kl':>9} | {'total':>9} | {'grad':>9}")
    print("-" * 76)
    ...
    for it in range(1, args.rollouts + 1):
        ...
        m = wm.train_world_model_on_rollout(batch, epochs=args.wm_epochs)
    ...
    # recon must fall below its starting value, and total must fall.
